...
sap.ui.define(['./Select', 'sap/ui/core/InvisibleText', 'sap/ui/core/Core', './ActionSelectRenderer'],
	function(Select, InvisibleText, Core, ActionSelectRenderer) {
		...
		ActionSelect.prototype._updateTutorMessage = function() {
			...
			if (!this._sTutormessageType) {
				this._sTutormessageType = this._getTutormessageType();
				this._oTutorMessageText = new InvisibleText(this._sTutormessageType, {
					text: Core.getLibraryResourceBundle("sap.m").getText("ACTION_SELECT_TUTOR_MESSAGE")
				}).toStatic();
			}

			bTutorMessageNotReferenced = (aAriaLabels.indexOf(this._sTutormessageType) === -1);

			if (bTutorMessageNotReferenced && bHasButtons) {
				oPicker.addAriaLabelledBy(this._sTutormessageType);
			} else {
				if (!bHasButtons) {
					oPicker.removeAriaLabelledBy(this._sTutormessageType);
				}
			}
		};
		 ...
		 * @private
		 */
		ActionSelect.prototype._getTutormessageType = function() {
