...
public class DateUtil {
     ...
     * @return
     */
    public static LocalDateTime dateTimeAdd(LocalDateTime date, Integer time, String unit) {
        ChronoUnit chronoUnit;
        if (TIME_UNIT_MINUTE.equalsIgnoreCase(unit)) {
            chronoUnit = ChronoUnit.MINUTES;
        } else if (TIME_UNIT_MINUTE.equalsIgnoreCase(unit)) {
            chronoUnit = ChronoUnit.HOURS;
        } else if (TIME_UNIT_DAY.equalsIgnoreCase(unit)) {
            chronoUnit = ChronoUnit.DAYS;
        } else if (TIME_UNIT_WEEK.equalsIgnoreCase(unit)) {
            chronoUnit = ChronoUnit.WEEKS;
        } else if (TIME_UNIT_MONTH.equalsIgnoreCase(unit)) {
            chronoUnit = ChronoUnit.MONTHS;
        } else {
            chronoUnit = ChronoUnit.YEARS;
        }
        if (time > 0) {
            return date.plus(time, chronoUnit);
        } else if (time < 0) {
            return date.minus(time, chronoUnit);
        } else {
            return date;
        }
    }
